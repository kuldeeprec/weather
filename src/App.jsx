    ...
    <div className="Background">
      <div className="mainWindow">
        <Button
          variant="outlined"
          onClick={() => openDialog()}
          style={{ fontWeight: "bold", width: 100, background: "#ffffff" }}
        >
          Show Whether
        </Button>
        {city.name !== undefined ? (
        ...
        <div className="middlePortion">
          {data !== [] ? (
            <div className="left">
              ...
              />
              <div>{data[0].weather[0].description}</div>
            </div>
          ) : null}

          <div className="centre">{Temprature}</div>
          <div className="right">
            <div>Wind: {data[0].wind.speed}</div>
            <div>Precip: {data[0].pop}</div>
            <div>Pressure: {data[0].main.pressure}</div>
          </div>
        </div>
        <div className="endPosition">
          {data.map((c, index) => (
            <div className={`img${index + 1}`}>
              ...
              <diV style={{ fontSize: 10 }}>{c.main.temp - 273}</diV>
            </div>
          ))}
          {/* <div className="img1">
